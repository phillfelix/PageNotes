...
(s.setVariable("textAlign","left"),m=a.left,a=c.bottom,null!==b&&0<w.trimText(b).length&&s.fillText(b,m,a))}}function A(m){return(m=d[m])&&!0===m.succeeded?m.img:!1}function E(m,s){var b=Math.max(m.left,s.left),a=Math.max(m.top,s.top),d=Math.min(m.left+m.width,s.left+s.width),c=Math.min(m.top+m.height,s.top+s.height);return{left:b,top:a,width:d-b,height:c-a}}function l(m,s,a,d,c){var f=b(s,"paddingLeft"),e=b(s,"paddingTop"),g=b(s,"paddingRight");s=b(s,"paddingBottom");t(m,a,0,0,a.width,a.height,d.left+
f+c[3].width,d.top+e+c[0].width,d.width-(c[1].width+c[3].width+f+g),d.height-(c[0].width+c[2].width+e+s))}function z(m){return["Top","Right","Bottom","Left"].map(function(a){return{width:b(m,"border"+a+"Width"),color:u(m,"border"+a+"Color")}})}function H(m){return["TopLeft","TopRight","BottomRight","BottomLeft"].map(function(a){return u(m,"border"+a+"Radius")})}function D(m,a,b,c){var d=function(m,a,b){return{x:m.x+(a.x-m.x)*b,y:m.y+(a.y-m.y)*b}};return{start:m,startControl:a,endControl:b,end:c,subdivide:function(f){var e=
...
t=a.top;k=a.width;h=a.height;d=x[0][0];f=x[0][1];e=x[1][0];g=x[1][1];var w=x[2][0],D=x[2][1];v=x[3][0];r=x[3][1];var O=k-e,A=h-D,E=k-w,y=h-r;c=I(l,t,d,f).topLeft.subdivide(.5);d=I(l+b[3].width,t+b[0].width,Math.max(0,d-b[3].width),Math.max(0,f-b[0].width)).topLeft.subdivide(.5);f=I(l+O,t,e,g).topRight.subdivide(.5);e=I(l+Math.min(O,k+b[3].width),t+b[0].width,O>k+b[3].width?0:e-b[3].width,g-b[0].width).topRight.subdivide(.5);g=I(l+E,t+A,w,D).bottomRight.subdivide(.5);k=I(l+Math.min(E,k+b[3].width),
t+Math.min(A,h+b[0].width),Math.max(0,w-b[1].width),Math.max(0,D-b[2].width)).bottomRight.subdivide(.5);h=I(l,t+y,v,r).bottomLeft.subdivide(.5);l=I(l+b[3].width,t+y,Math.max(0,v-b[3].width),Math.max(0,r-b[2].width)).bottomLeft.subdivide(.5);t=[];switch(u(m,"backgroundClip")){case "content-box":case "padding-box":G(t,x[0],x[1],d,e,a.left+b[3].width,a.top+b[0].width);G(t,x[1],x[2],e,k,a.left+a.width-b[1].width,a.top+b[0].width);G(t,x[2],x[3],k,l,a.left+a.width-b[1].width,a.top+a.height-b[2].width);
...
stack:c}}()};h.Preload=function(d){function c(){A.log("html2canvas: start: images: "+f.numLoaded+" / "+f.numTotal+" (failed: "+f.numFailed+")");!f.firstRun&&f.numLoaded>=f.numTotal&&(A.log("Finished loading images: # "+f.numTotal+" (failed: "+f.numFailed+")"),"function"===typeof d.complete&&d.complete(f))}function b(a,b,g){var k,h=d.proxy,l;B.href=a;a=B.href;k="html2canvas_"+z++;g.callbackname=k;h=-1<h.indexOf("?")?h+"&":h+"?";h+="url="+encodeURIComponent(a)+"&callback="+k;l=H.createElement("script");
...
a.method&&a.args&&0<a.args.length}function n(a,b){var d;h.Util.parseBackgroundImage(a).filter(k).forEach(function(a){if("url"===a.method)E.loadImage(a.args[0]);else if(a.method.match(/\-?gradient$/)){d===q&&(d=h.Util.Bounds(b));a=a.value;var e=h.Generate.Gradient(a,d);e!==q&&(f[a]={img:e,succeeded:!0},f.numTotal++,f.numLoaded++,c())}})}function a(b){var c=!1;try{A.Children(b).forEach(a)}catch(d){}try{c=b.nodeType}catch(f){c=!1,A.log("html2canvas: failed to access some element's nodeType - Exception: "+
f.message)}if(1===c||c===q){g(b,":before");g(b,":after");try{n(A.getCSS(b,"backgroundImage"),b)}catch(e){A.log("html2canvas: failed to get background-image - Exception: "+e.message)}n(b)}}function v(a,g){a.onload=function(){g.timer!==q&&e.clearTimeout(g.timer);f.numLoaded++;g.succeeded=!0;a.onerror=a.onload=null;c()};a.onerror=function(){if("anonymous"===a.crossOrigin&&(e.clearTimeout(g.timer),d.proxy)){var k=a.src;a=new Image;g.img=a;a.src=k;b(a.src,a,g);return}f.numLoaded++;f.numFailed++;g.succeeded=
!1;a.onerror=a.onload=null;c()}}var f={numLoaded:0,numFailed:0,numTotal:0,cleanupDone:!1},r,A=h.Util,E,l,z=0;l=d.elements[0]||p.body;var H=l.ownerDocument,D=l.getElementsByTagName("img"),G=D.length,B=H.createElement("a"),F=function(a){return a.crossOrigin!==q}(new Image),K;B.href=e.location.href;r=B.protocol+B.host;E={loadImage:function(a){var c,e;a&&f[a]===q&&(c=new Image,a.match(/data:image\/.*;base64,/i)?(c.src=a.replace(/url\(['"]{0,}|['"]{0,}\)$/ig,""),e=f[a]={img:c},f.numTotal++,v(c,e)):(B.href=
a,B.href=B.href,B.protocol+B.host===r||!0===d.allowTaint?(e=f[a]={img:c},f.numTotal++,v(c,e),c.src=a):F&&!d.allowTaint&&d.useCORS?(c.crossOrigin="anonymous",e=f[a]={img:c},f.numTotal++,v(c,e),c.src=a):d.proxy&&(e=f[a]={img:c},f.numTotal++,b(a,c,e))))},cleanupDOM:function(a){var b,g;if(!f.cleanupDone){a&&"string"===typeof a?A.log("html2canvas: Cleanup because: "+a):A.log("html2canvas: Cleanup after timeout: "+d.timeout+" ms.");for(g in f)if(f.hasOwnProperty(g)&&(b=f[g],"object"===typeof b&&b.callbackname&&
b.succeeded===q)){e[b.callbackname]=q;try{delete e[b.callbackname]}catch(k){}b.script&&b.script.parentNode&&(b.script.setAttribute("src","about:blank"),b.script.parentNode.removeChild(b.script));f.numLoaded++;f.numFailed++;A.log("html2canvas: Cleaned up failed img: '"+g+"' Steps: "+f.numLoaded+" / "+f.numTotal)}e.stop!==q?e.stop():p.execCommand!==q&&p.execCommand("Stop",!1);p.close!==q&&p.close();f.cleanupDone=!0;a&&"string"===typeof a||c()}},renderingDone:function(){K&&e.clearTimeout(K)}};0<d.timeout&&
(K=e.setTimeout(E.cleanupDOM,d.timeout));A.log("html2canvas: Preload starts: finding background-images");f.firstRun=!0;a(l);A.log("html2canvas: Preload: Finding images");for(l=0;l<G;l+=1)E.loadImage(D[l].getAttribute("src"));f.firstRun=!1;A.log("html2canvas: Preload: Done.");f.numTotal===f.numLoaded&&c();return E};h.Renderer=function(d,c){return function(b){if("string"===typeof c.renderer&&h.Renderer[b]!==q)b=h.Renderer[b](c);else if("function"===typeof b)b=b(c);else throw Error("Unknown renderer");
...
p){var q=this.root,r=new createjs.Shape;r.x=e;r.y=p;r.graphics.drawRect(0,0,1,1);var z=new createjs.Bitmap(q.screenshot);z.mask=r;q.stage.addChild(z);q.stage.update();q.temp.rectangle_highlight={mask:r,display:z}},updateMask:function(e,p){var q=this.root,r=q.temp.rectangle_highlight,z=e-r.mask.x,F=p-r.mask.y;r.mask.graphics.clear();r.mask.graphics.drawRect(0,0,z,F);q.stage.update()},saveMask:function(){this.root.temp.rectangle_highlight=!1}}})();
...
(function(){window.PagenotesDefaultToolkit=window.PagenotesDefaultToolkit||{};window.PagenotesDefaultToolkit.text={on:function(e){this.root=e;var p=this;e.canvas.on("click",function(e){e.preventDefault();p.textField(e.offsetX,e.offsetY)})},off:function(e){e.canvas.off("click")},textField:function(e,p){var q=this,r=this.root;if(!r.temp.text){var z=$("<textarea></textarea>").css({position:"fixed",left:e,top:p}).on("keydown",function(z){13!=z.keyCode||z.shiftKey||(q.saveText($(this).val(),e,p),$(this).remove(),
r.temp.text=!1)}).appendTo($(document.body)).focus();r.temp.text=z}},saveText:function(e,p,q){var r=this.root;e=new createjs.Text(e,"14px Arial","#ddd");e.x=p;e.y=q;r.stage.addChild(e);r.stage.update()}}})();
...
q.width).attr("height",q.height).appendTo($(document.body));e.stage=new createjs.Stage("pagenotes-canvas");e.screenshot=q;var r=new createjs.Bitmap(q);r.filters=[new createjs.BlurFilter(5,5,1)];r.cache(0,0,q.width,q.height);var z=new createjs.Shape;z.graphics.beginFill("rgba(0, 0, 0, 0.6)").drawRect(0,0,q.width,q.height);e.stage.addChild(r);e.stage.addChild(z);e.stage.update();e.selectTool("rectangle_highlight");e.renderToolbar()}})},renderToolbar:function(){var e=this,q=$('<div class="pagenotes-toolbar"></div>').appendTo($(document.body));
$.each(this.toolkit,function(e,p){$('<button data-tool="'+e+'">'+e+"</button>").appendTo(q)});$(".pagenotes-toolbar button").on("click",function(q){q.preventDefault();e.selectTool($(this).data("tool"))})},selectTool:function(e){this.activeTool&&this.toolkit[this.activeTool].off(this);this.toolkit[e]&&(this.activeTool=e,this.toolkit[e].on(this))}};(new e(PagenotesDefaultToolkit)).init()})();