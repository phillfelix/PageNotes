...
name:"drawImage",arguments:arguments})},fillText:function(){b.push({type:"function",name:"fillText",arguments:arguments})},setVariable:function(d,c){b.push({type:"variable",name:d,arguments:c});return c}}}var h={},M;h.Util={};h.Util.log=function(d){h.logging&&e.console&&e.console.log&&e.console.log(d)};h.Util.trimText=function(d){return function(c){return d?d.apply(c):((c||"")+"").replace(/^\s+|\s+$/g,"")}}(String.prototype.trim);h.Util.asFloat=function(d){return parseFloat(d)};(function(){var d=
/((rgba|rgb)\([^\)]+\)(\s-?\d+px){0,})/g,c=/(-?\d+px)|(#.+)|(rgb\(.+\))|(rgba\(.+\))/g;h.Util.parseTextShadows=function(b){if(!b||"none"===b)return[];b=b.match(d);for(var g=[],k=0;b&&k<b.length;k++){var n=b[k].match(c);g.push({color:n[0],offsetX:n[1]?n[1].replace("px",""):0,offsetY:n[2]?n[2].replace("px",""):0,blur:n[3]?n[3].replace("px",""):0})}return g}})();h.Util.parseBackgroundImage=function(d){var c,b,g,k,n,a=[],v,f=0,H=0,e,h,l=function(){c&&('"'===b.substr(0,1)&&(b=b.substr(1,b.length-2)),b&&
h.push(b),"-"===c.substr(0,1)&&0<(k=c.indexOf("-",1)+1)&&(g=c.substr(0,k),c=c.substr(k)),a.push({prefix:g,method:c.toLowerCase(),value:n,args:h}));h=[];c=g=b=n=""};l();for(var p=0,q=d.length;p<q;p++)if(v=d[p],!(0===f&&-1<" \r\n\t".indexOf(v))){switch(v){case '"':e?e===v&&(e=null):e=v;break;case "(":if(e)break;else if(0===f){f=1;n+=v;continue}else H++;break;case ")":if(e)break;else if(1===f)if(0===H){f=0;n+=v;l();continue}else H--;break;case ",":if(!e)if(0===f){l();continue}else if(1===f&&0===H&&!c.match(/^url$/i)){h.push(b);
...
u(s,"fontVariant"),a,f,d].join(" "));m.setVariable("textAlign","left");e.length&&(m.setVariable("shadowColor",e[0].color),m.setVariable("shadowOffsetX",e[0].offsetX),m.setVariable("shadowOffsetY",e[0].offsetY),m.setVariable("shadowBlur",e[0].blur));if("none"!==b)return w.Font(d,f,y)}function f(m,s,b){var d=b.ctx,f=u(m,"color"),e=u(m,"textDecoration"),k=u(m,"textAlign"),h,l,p=s,D=0;0<w.trimText(s.nodeValue).length&&(s.nodeValue=n(s.nodeValue,u(m,"textTransform")),k=k.replace(["-webkit-auto"],["auto"]),
l=!c.letterRendering&&/^(left|right|justify|auto)$/.test(k)&&a(u(m,"letterSpacing"))?s.nodeValue.split(/(\b| )/):s.nodeValue.split(""),h=v(d,m,e,f),c.chinese&&l.forEach(function(m,s){/.*[\u4E00-\u9FA5].*$/.test(m)&&(m=m.split(""),m.unshift(s,1),l.splice.apply(l,m))}),l.forEach(function(m,s){var a,c=s<l.length-1;a=b.transform.matrix;var k;if(Y.rangeBounds&&!a){if("none"!==e||0!==w.trimText(m).length)a=p,c=D,k=y.createRange(),k.setStart(a,c),k.setEnd(a,c+m.length),k=k.getBoundingClientRect();D+=m.length}else if(p&&
...
f+c[3].width,d.top+e+c[0].width,d.width-(c[1].width+c[3].width+f+g),d.height-(c[0].width+c[2].width+e+s))}function A(m){return["Top","Right","Bottom","Left"].map(function(a){return{width:b(m,"border"+a+"Width"),color:u(m,"border"+a+"Color")}})}function I(m){return["TopLeft","TopRight","BottomRight","BottomLeft"].map(function(a){return u(m,"border"+a+"Radius")})}function C(m,a,b,c){var d=function(m,a,b){return{x:m.x+(a.x-m.x)*b,y:m.y+(a.y-m.y)*b}};return{start:m,startControl:a,endControl:b,end:c,subdivide:function(f){var e=
d(m,a,f),g=d(a,b,f),k=d(b,c,f),h=d(e,g,f),g=d(g,k,f);f=d(h,g,f);return[C(m,e,h,f),C(f,g,k,c)]},curveTo:function(m){m.push(["bezierCurve",a.x,a.y,b.x,b.y,c.x,c.y])},curveToReversed:function(c){c.push(["bezierCurve",b.x,b.y,a.x,a.y,m.x,m.y])}}}function G(m,a,b,c,d,f,e){0<a[0]||0<a[1]?(m.push(["line",c[0].start.x,c[0].start.y]),c[0].curveTo(m),c[1].curveTo(m)):m.push(["line",f,e]);(0<b[0]||0<b[1])&&m.push(["line",d[0].start.x,d[0].start.y])}function B(m,a,b,c,d,f,e){var g=[];0<a[0]||0<a[1]?(g.push(["line",
c[1].start.x,c[1].start.y]),c[1].curveTo(g)):g.push(["line",m.c1[0],m.c1[1]]);0<b[0]||0<b[1]?(g.push(["line",f[0].start.x,f[0].start.y]),f[0].curveTo(g),g.push(["line",e[0].end.x,e[0].end.y]),e[0].curveToReversed(g)):(g.push(["line",m.c2[0],m.c2[1]]),g.push(["line",m.c3[0],m.c3[1]]));0<a[0]||0<a[1]?(g.push(["line",d[1].end.x,d[1].end.y]),d[1].curveToReversed(g)):g.push(["line",m.c4[0],m.c4[1]]);return g}function F(m,a,b){var c,d,f,e,g,k,h,l,n=a.left,p=a.top,D=a.width,q=a.height,t,v,r,K,x=I(m);l=a.left;
t=a.top;k=a.width;h=a.height;d=x[0][0];f=x[0][1];e=x[1][0];g=x[1][1];var H=x[2][0],w=x[2][1];v=x[3][0];r=x[3][1];var C=k-e,z=h-w,E=k-H,y=h-r;c=J(l,t,d,f).topLeft.subdivide(.5);d=J(l+b[3].width,t+b[0].width,Math.max(0,d-b[3].width),Math.max(0,f-b[0].width)).topLeft.subdivide(.5);f=J(l+C,t,e,g).topRight.subdivide(.5);e=J(l+Math.min(C,k+b[3].width),t+b[0].width,C>k+b[3].width?0:e-b[3].width,g-b[0].width).topRight.subdivide(.5);g=J(l+E,t+z,H,w).bottomRight.subdivide(.5);k=J(l+Math.min(E,k+b[3].width),
t+Math.min(z,h+b[0].width),Math.max(0,H-b[1].width),Math.max(0,w-b[2].width)).bottomRight.subdivide(.5);h=J(l,t+y,v,r).bottomLeft.subdivide(.5);l=J(l+b[3].width,t+y,Math.max(0,v-b[3].width),Math.max(0,r-b[2].width)).bottomLeft.subdivide(.5);t=[];switch(u(m,"backgroundClip")){case "content-box":case "padding-box":G(t,x[0],x[1],d,e,a.left+b[3].width,a.top+b[0].width);G(t,x[1],x[2],e,k,a.left+a.width-b[1].width,a.top+b[0].width);G(t,x[2],x[3],k,l,a.left+a.width-b[1].width,a.top+a.height-b[2].width);
G(t,x[3],x[0],l,d,a.left+b[3].width,a.top+a.height-b[2].width);break;default:G(t,x[0],x[1],c,f,a.left,a.top),G(t,x[1],x[2],f,g,a.left+a.width,a.top),G(t,x[2],x[3],g,h,a.left+a.width,a.top+a.height),G(t,x[3],x[0],h,c,a.left,a.top+a.height)}y={clip:t,borders:[]};for(m=0;4>m;m++)if(0<b[m].width){a=n;t=p;v=D;r=q-b[2].width;switch(m){case 0:r=b[0].width;K=B({c1:[a,t],c2:[a+v,t],c3:[a+v-b[1].width,t+r],c4:[a+b[3].width,t+r]},x[0],x[1],c,d,f,e);break;case 1:a=n+D-b[1].width;v=b[1].width;K=B({c1:[a+v,t],
c2:[a+v,t+r+b[2].width],c3:[a,t+r],c4:[a,t+b[0].width]},x[1],x[2],f,e,g,k);break;case 2:t=t+q-b[2].width;r=b[2].width;K=B({c1:[a+v,t+r],c2:[a,t+r],c3:[a+b[3].width,t],c4:[a+v-b[3].width,t]},x[2],x[3],g,k,h,l);break;case 3:v=b[3].width,K=B({c1:[a,t+r+b[2].width],c2:[a,t],c3:[a+v,t+b[0].width],c4:[a+v,t+r]},x[3],x[0],h,l,c,d)}y.borders.push({args:K,color:b[m].color})}return y}function L(a,b){var c=a.drawShape();b.forEach(function(a,b){c[0===b?"moveTo":a[0]+"To"].apply(null,a.slice(1))});return c}function D(a,
...
u(a,"height");d.style.top=b.top+"px";d.style.left=b.left+"px";b="SELECT"===a.nodeName?(a.options[a.selectedIndex]||0).text:a.value;b||(b=a.placeholder);b=y.createTextNode(b);d.appendChild(b);N.appendChild(d);f(a,b,c);N.removeChild(d)}function t(a){a.drawImage.apply(a,Array.prototype.slice.call(arguments,1))}function K(a,b){var d=e.getComputedStyle(a,b);if(d&&d.content&&"none"!==d.content&&"-moz-alt-content"!==d.content&&"none"!==d.display){var c=d.content+"",f=c.substr(0,1);f===c.substr(c.length-
1)&&f.match(/'|"/)&&(c=c.substr(1,c.length-2));var f="url"===c.substr(0,3),g=p.createElement(f?"img":"span");g.className="___html2canvas___pseudoelement-before ___html2canvas___pseudoelement-after";Object.keys(d).filter(Z).forEach(function(a){try{g.style[a]=d[a]}catch(b){w.log(["Tried to assign readonly property ",a,"Error:",b])}});f?g.src=w.parseBackgroundImage(c)[0].args[0]:g.innerHTML=c;return g}}function Z(a){return isNaN(e.parseInt(a,10))}function aa(a,b){var d=K(a,":before"),c=K(a,":after");
...
d);a.fill();a.translate(-f,-d)}function Q(a,b,d,c,f,e,g,k){var h=[];h.push(["line",Math.round(f),Math.round(e)]);h.push(["line",Math.round(f+g),Math.round(e)]);h.push(["line",Math.round(f+g),Math.round(k+e)]);h.push(["line",Math.round(f),Math.round(k+e)]);L(a,h);a.save();a.clip();S(a,b,d,c);a.restore()}function M(a,b,d){for(var c=u(a,"backgroundImage"),f=w.parseBackgroundImage(c),e,g=f.length;g--;)if(c=f[g],c.args&&0!==c.args.length)if(e=z("url"===c.method?c.args[0]:c.value)){var k=a,c=b,h=d,l=e;
e=g;var n=w.BackgroundSize(k,c,l,e),p=w.BackgroundPosition(k,c,l,e,n),k=u(k,"backgroundRepeat").split(",").map(w.trimText);if(l.width!==n.width||l.height!==n.height){var D=void 0,q=y.createElement("canvas");q.width=n.width;q.height=n.height;D=q.getContext("2d");t(D,l,0,0,l.width,l.height,0,0,n.width,n.height);l=q}k=k[e]||k[0];switch(k){case "repeat-x":Q(h,l,p,c,c.left,c.top+p.top,99999,l.height);break;case "repeat-y":Q(h,l,p,c,c.left+p.left,c.top,l.width,99999);break;case "no-repeat":Q(h,l,p,c,c.left+
...
n=fa(f,e,b.clip),t=F(a,e,f),p=U.test(a.nodeName)?"#efefef":u(a,"backgroundColor");L(h,t.clip);h.save();h.clip();0<n.height&&0<n.width&&!d?(g(h,e.left,e.top,e.width,e.height,p),M(a,n,h)):d&&(b.backgroundColor=p);h.restore();t.borders.forEach(function(a){var b=a.args;a=a.color;"transparent"!==a&&(h.setVariable("fillStyle",a),L(h,b),h.fill())});c||aa(a,b);switch(a.nodeName){case "IMG":(k=z(a.getAttribute("src")))?l(h,a,k,e,f):w.log("html2canvas: Error loading <img>:"+a.getAttribute("src"));break;case "INPUT":/^(text|url|email|submit|button|reset)$/.test(a.type)&&
0<(a.value||a.placeholder||"").length&&D(a,e,b);break;case "TEXTAREA":0<(a.value||a.placeholder||"").length&&D(a,e,b);break;case "SELECT":0<(a.options||a.placeholder||"").length&&D(a,e,b);break;case "LI":r(a,b,n);break;case "CANVAS":l(h,a,a,e,f)}return b}function P(a,b,c){"none"===u(a,"display")||"hidden"===u(a,"visibility")||a.hasAttribute("data-html2canvas-ignore")||(b=T(a,b,c,!1)||b,U.test(a.nodeName)||V(a,b,c))}function V(a,b,c){w.Children(a).forEach(function(d){d.nodeType===d.ELEMENT_NODE?P(d,
b,c):d.nodeType===d.TEXT_NODE&&f(a,d,b)})}e.scroll(0,0);var O=c.elements===q?p.body:c.elements[0],y=O.ownerDocument,w=h.Util,Y=w.Support(c,y),U=new RegExp("("+c.ignoreElements+")"),N=y.body,u=w.getCSS,R=y.createElement("style");R.innerHTML='.___html2canvas___pseudoelement-before:before { content: "" !important; display: none !important; }.___html2canvas___pseudoelement-after:after { content: "" !important; display: none !important; }';N.appendChild(R);d=d||{};var J=function(a){return function(b,c,
d,f){var e=d*a,g=f*a;d=b+d;f=c+f;return{topLeft:C({x:b,y:f},{x:b,y:f-g},{x:d-e,y:c},{x:d,y:c}),topRight:C({x:b,y:c},{x:b+e,y:c},{x:d,y:f-g},{x:d,y:f}),bottomRight:C({x:d,y:c},{x:d,y:c+g},{x:b+e,y:f},{x:b,y:f}),bottomLeft:C({x:d,y:f},{x:d-e,y:f},{x:b,y:c+g},{x:b,y:c})}}}(4*((Math.sqrt(2)-1)/3)),da=/(matrix)\((.+)\)/;return function(){var a=u(p.documentElement,"backgroundColor"),b=w.isTransparent(a)&&O===p.body,c=T(O,null,!1,b);V(O,c);b&&(a=c.backgroundColor);N.removeChild(R);return{backgroundColor:a,
stack:c}}()};h.Preload=function(d){function c(){z.log("html2canvas: start: images: "+f.numLoaded+" / "+f.numTotal+" (failed: "+f.numFailed+")");!f.firstRun&&f.numLoaded>=f.numTotal&&(z.log("Finished loading images: # "+f.numTotal+" (failed: "+f.numFailed+")"),"function"===typeof d.complete&&d.complete(f))}function b(a,b,g){var k,h=d.proxy,l;B.href=a;a=B.href;k="html2canvas_"+A++;g.callbackname=k;h=-1<h.indexOf("?")?h+"&":h+"?";h+="url="+encodeURIComponent(a)+"&callback="+k;l=I.createElement("script");
...
!1;a.onerror=a.onload=null;c()}}var f={numLoaded:0,numFailed:0,numTotal:0,cleanupDone:!1},r,z=h.Util,E,l,A=0;l=d.elements[0]||p.body;var I=l.ownerDocument,C=l.getElementsByTagName("img"),G=C.length,B=I.createElement("a"),F=function(a){return a.crossOrigin!==q}(new Image),L;B.href=e.location.href;r=B.protocol+B.host;E={loadImage:function(a){var c,e;a&&f[a]===q&&(c=new Image,a.match(/data:image\/.*;base64,/i)?(c.src=a.replace(/url\(['"]{0,}|['"]{0,}\)$/ig,""),e=f[a]={img:c},f.numTotal++,v(c,e)):(B.href=
...
b.succeeded===q)){e[b.callbackname]=q;try{delete e[b.callbackname]}catch(k){}b.script&&b.script.parentNode&&(b.script.setAttribute("src","about:blank"),b.script.parentNode.removeChild(b.script));f.numLoaded++;f.numFailed++;z.log("html2canvas: Cleaned up failed img: '"+g+"' Steps: "+f.numLoaded+" / "+f.numTotal)}e.stop!==q?e.stop():p.execCommand!==q&&p.execCommand("Stop",!1);p.close!==q&&p.close();f.cleanupDone=!0;a&&"string"===typeof a||c()}},renderingDone:function(){L&&e.clearTimeout(L)}};0<d.timeout&&
(L=e.setTimeout(E.cleanupDOM,d.timeout));z.log("html2canvas: Preload starts: finding background-images");f.firstRun=!0;a(l);z.log("html2canvas: Preload: Finding images");for(l=0;l<G;l+=1)E.loadImage(C[l].getAttribute("src"));f.firstRun=!1;z.log("html2canvas: Preload: Done.");f.numTotal===f.numLoaded&&c();return E};h.Renderer=function(d,c){return function(b){if("string"===typeof c.renderer&&h.Renderer[b]!==q)b=h.Renderer[b](c);else if("function"===typeof b)b=b(c);else throw Error("Unknown renderer");
...
c,d){var e="auto"===c.zIndex.zindex?0:Number(c.zIndex.zindex),g=b,k=c.zIndex.isPositioned,h=c.zIndex.isFloated,n={node:c},p=d;if(c.zIndex.ownStacking)g=n.context={"!":[{node:c,children:[]}]},p=q;else if(k||h)p=n.children=[];0===e&&d?d.push(n):(b[e]||(b[e]=[]),b[e].push(n));c.zIndex.children.forEach(function(b){a(g,b,p)})}var c={};a(c,b);return c}(b);c(b);return d}(d.stack),h)};h.Util.Support=function(d,c){function b(){var b=new Image,d=c.createElement("canvas"),e=d.getContext===q?!1:d.getContext("2d");
...
Math.ceil(h.height),l=r.getContext("2d"),l.drawImage(a,h.left,h.top,h.width,h.height,0,0,h.width,h.height),a=null,r):a}}})(window,document);(function(){var e=document.createElement("style");e.innerHTML+=".pagenotes-canvas {";e.innerHTML+="position: absolute; top: 0; left: 0;";e.innerHTML+="}";e.innerHTML+=".pagenotes-toolbar {";e.innerHTML+="position: fixed; left: 0; bottom: 0;";e.innerHTML+="padding: 10px;";e.innerHTML+="background: #ddd;";e.innerHTML+="z-index: 3;";e.innerHTML+="}";document.getElementsByTagName("head")[0].appendChild(e)})();
...
(function(){var e=function(e){this.temp={mask:!1};this.masks=[];this.toolkit=e||{}};e.prototype={init:function(){var e=this;$(document.body).outerWidth();$(document.body).outerHeight();this.canvas=$('<canvas id="pagenotes-canvas" class="pagenotes-canvas"></canvas>');html2canvas(document.body,{onrendered:function(q){e.canvas.attr("width",q.width).attr("height",q.height).appendTo($(document.body));e.stage=new createjs.Stage("pagenotes-canvas");e.screenshot=q;var r=new createjs.Bitmap(q);r.filters=[new createjs.BlurFilter(5,
5,1)];r.cache(0,0,q.width,q.height);var A=new createjs.Shape;A.graphics.beginFill("rgba(0, 0, 0, 0.6)").drawRect(0,0,q.width,q.height);e.stage.addChild(r);e.stage.addChild(A);e.stage.update();e.selectTool("rectangle_highlight");e.renderToolbar()}})},renderToolbar:function(){var e=this,q=$('<div id="pagenotes-toolbar" class="pagenotes-toolbar"></div>').appendTo($(document.body));$.each(this.toolkit,function(e,p){$('<button data-tool="'+e+'">'+e+"</button>").appendTo(q)});$("#pagenotes-toolbar button").on("click",
function(q){q.preventDefault();e.selectTool($(this).data("tool"))})},selectTool:function(e){this.activeTool&&this.toolkit[this.activeTool].off(this);this.toolkit[e]&&(this.activeTool=e,this.toolkit[e].on(this))}};(new e(PagenotesDefaultToolkit)).init()})();